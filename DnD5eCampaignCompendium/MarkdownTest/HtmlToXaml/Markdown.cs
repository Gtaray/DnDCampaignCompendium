...
namespace MarkdownTest.MarkdownToXaml
{
    public class Markdown : DependencyObject
    {
        ...
        {
            get { return (Style)GetValue(BodyStyleProperty); }
            set { SetValue(BodyStyleProperty, value); }
        }

        // Using a DependencyProperty as the backing store for BodyStyle.  This enables animation, styling, binding, etc...
        public static readonly DependencyProperty BodyStyleProperty =
            DependencyProperty.Register("BodyStyle", typeof(Style), typeof(Markdown), new PropertyMetadata(null));
        ...
        public Style HalfTableStyle
        {
            get { return (Style)GetValue(HalfTableStyleProperty); }
            set { SetValue(HalfTableStyleProperty, value); }
        }

        // Using a DependencyProperty as the backing store for HalfTableStyle.  This enables animation, styling, binding, etc...
        public static readonly DependencyProperty HalfTableStyleProperty =
            DependencyProperty.Register("HalfTableStyle", typeof(Style), typeof(Markdown), new PropertyMetadata(null));

        public Style TableHeaderRowStyle
        {
            get { return (Style)GetValue(TableHeaderRowStyleProperty); }
            set { SetValue(TableHeaderRowStyleProperty, value); }
        }

        // Using a DependencyProperty as the backing store for TableHeaderRowStyle.  This enables animation, styling, binding, etc...
        public static readonly DependencyProperty TableHeaderRowStyleProperty =
            DependencyProperty.Register("TableHeaderRowStyle", typeof(Style), typeof(Markdown), new PropertyMetadata(null));
        ...
        // Using a DependencyProperty as the backing store for TableRowStyle.  This enables animation, styling, binding, etc...
        ...
        public Color TableRowAlternatingColor
        {
            get { return (Color)GetValue(TableRowAlternatingColorProperty); }
            set { SetValue(TableRowAlternatingColorProperty, value); }
        }

        // Using a DependencyProperty as the backing store for TableRowAlternatingColor.  This enables animation, styling, binding, etc...
        public static readonly DependencyProperty TableRowAlternatingColorProperty =
            DependencyProperty.Register("TableRowAlternatingColor", typeof(Color), typeof(Markdown), new PropertyMetadata(null));
    ...
    RegexOptions.Multiline | RegexOptions.IgnorePatternWhitespace | RegexOptions.Compiled);
        ...
        private Block ListEvaluator(Match match)
        {
            ...
            resultList.MarkerOffset = 16;
            resultList.Margin = new Thickness(20, 0, 0, 0);
        ...
        #region Tables
        private static Regex _tables = new Regex(@"
            (_{3})?
            [\r?\n]*\|(?:([^\r\n\|]*)\|)+      # table header row
            \r?\n\| (?:(\:?[-|*]+\:?)\|)+   # alignment row
            \r?\n(\|(?:([^\r\n\|]*)\|)+     # all rows
        ...
        private IEnumerable<Block>DoTables(string text, Func<string, IEnumerable<Block>> defaultHandler)
        {
            ...
            return Evaluate(text, _tables, TableEvaluator, defaultHandler);
        }
        ...
        private Block TableEvaluator(Match match)
        {
            ...
            TableColumnDefinition[] columns = GetTableColumnDefinitions(
                match.Value.Substring(match.Groups[1].Length, match.Value.Length - match.Groups[1].Length));

            int colCount = columns.Length;
            int rowCount = columns[0].Content.Count() + 1;  // content rows + header row

            Grid table = new Grid();
            table.Style = match.Groups[1].Success ? TableStyle : HalfTableStyle;

            for (int c = 0; c < colCount; c++)
            {
                ColumnDefinition cd = new ColumnDefinition();
                cd.Width = columns[c].Width;
            ...
            SolidColorBrush alternatingBrush = new SolidColorBrush(TableRowAlternatingColor);
            foreach (var column in columns)
            {
                // Header
                TextBlock header = Create<TextBlock, Inline>(RunSpanGamut(column.Header));
                header.TextAlignment = column.Alignment;
                header.Style = TableHeaderRowStyle;
                int columnIndex = Array.IndexOf(columns, column);
                Grid.SetColumn(header, columnIndex);
                Grid.SetRow(header, 0);
                table.Children.Add(header);

                for(int r = 0; r < column.Content.Count(); r++)
                {
                    TextBlock content = Create<TextBlock, Inline>(RunSpanGamut(column.Content[r]));
                    content.TextAlignment = column.Alignment;
                    content.Style = TableRowStyle;
                    if (TableRowAlternatingColor != null && r % 2 == 1) content.Background = alternatingBrush;
                    Grid.SetRow(content, r + 1);
                    Grid.SetColumn(content, columnIndex);
                    table.Children.Add(content);
                }
            }

            return new BlockUIContainer(table);
        }

        private TableColumnDefinition[] GetTableColumnDefinitions(string text)
        {
            List<TableColumnDefinition> columns = new List<TableColumnDefinition>();

            string[][] cells = text.Split(new string[] { "\r\n", "\n" }, StringSplitOptions.RemoveEmptyEntries)
                .Select(l => l.Split(new string[] { "|" }, StringSplitOptions.RemoveEmptyEntries))
                .ToArray();
            int rowCount = cells.Length;
            int colCount = cells[0].Length;

            //int colCount = lines[0].Split(new string[] { "|" }, StringSplitOptions.RemoveEmptyEntries).Length;

            for(int c = 0; c < colCount; c++)
            {
                TableColumnDefinition tcd = new TableColumnDefinition();
                tcd.Header = cells[0][c].Trim();
                tcd.Alignment = GetTableColumnAlignment(cells[1][c].Trim());
                tcd.Width = GetTableColumnWidth(cells[1][c].Trim());

                // all rows
                // Start for loop at 2 as that's the line index where content starts
                // after header and alignment rows
                tcd.Content = new List<string>();
                for (int r = 2; r < rowCount; r++)
                    tcd.Content.Add(cells[r][c].Trim());
                columns.Add(tcd);
            }
        ...
        private TextAlignment GetTableColumnAlignment(string text)
        {
        ...
        private GridLength GetTableColumnWidth(string text)
        {
            int starCount = text.Count(c => c == '*');
            if(starCount == 0)
                return new GridLength(0, GridUnitType.Auto);

            return new GridLength(starCount, GridUnitType.Star);
        }
        #endregion
        ...
        private IEnumerable<Block> FormParagraphs(string text)
        {
            ...
            {
                var paragraph = Create<Paragraph, Inline>(RunSpanGamut(g));
                if(BodyStyle != null)
        ...
        #region Nested brackets and parenthesis
